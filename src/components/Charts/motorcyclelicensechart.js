...
import options from './licenseoptions';
...
function MotorcycleLicenseChart(props) {
   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
      for(var k=0;k<survey[i].families[j].members.length;k++)
      {
        ...
        //   age_bracket[''].count++;
        // }
        if(options.hasOwnProperty(survey[i].families[j].members[k].twoWheelerLicense))
        options[survey[i].families[j].members[k].twoWheelerLicense].count++;
        else
        nulls++;
...
function download_png() {
  ...
  return (
        ...
        <div id='twowheeler_license' className="charts-header">
         { chart_type===0  &&
...
export default MotorcycleLicenseChart;
