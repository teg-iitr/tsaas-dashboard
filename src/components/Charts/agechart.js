...
import options from './age_bracket';
   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
      for(var k=0;k<survey[i].families[j].members.length;k++)
      {
        ...
        //   age_bracket[''].count++;
        // }
        if(options.hasOwnProperty(survey[i].families[j].members[k].age))
        options[survey[i].families[j].members[k].age].count++;
        else
        nulls++;
...
function download_png() {
  ...
  return (
        ...
        <div id='age_chart' className="charts-header">
          { chart_type===0  &&
