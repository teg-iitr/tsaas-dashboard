...
function CarLicenseChart(props) {
   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
      for(var k=0;k<survey[i].families[j].members.length;k++)
      {
        ...
        //   age_bracket[''].count++;
        // }
        if(options.hasOwnProperty(survey[i].families[j].members[k].fourWheelerLicense))
        options[survey[i].families[j].members[k].fourWheelerLicense].count++;
        else
        nulls++;
...
function download_png() {
  ...
  return (
        ...
        <div id='car_license' className="charts-header">
          { chart_type===0  &&
...
export default CarLicenseChart;
