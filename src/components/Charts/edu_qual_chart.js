...
import options from './edu_qual_options';
// import html2canvas from "html2canvas";
const pdfConverter = require("jspdf"); 
function EduQualChart(props) {
   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
      for(var k=0;k<survey[i].families[j].members.length;k++)
      {
        ...
        //   age_bracket[''].count++;
        // }
        if(options.hasOwnProperty(survey[i].families[j].members[k].educationalQualification))
        options[survey[i].families[j].members[k].educationalQualification].count++;
        else
        nulls++;
...
function download_png() {
  ...
  return (
        ...
        <div id='edu_qual' className="charts-header">
          { chart_type===0  &&
