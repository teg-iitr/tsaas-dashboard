   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
      ...
      {
        for(var l=0;l<survey[i].families[j].members[k].trips.length;l++)
        {
            for(var m=0;m<survey[i].families[j].members[k].trips[l].mode_types.length;m++)
            {
              if(survey[i].families[j].members[k].trips[l].mode_types[m].modeName!==null)
            { 
            //   if(!trips.hasOwnProperty(survey[i].families[j].members[k].trips[l].mode_types[m].modeName.toLowerCase()))
            // trips[survey[i].families[j].members[k].trips[l].mode_types[m].modeName.toLowerCase()]=0;
            // trips[survey[i].families[j].members[k].trips[l].mode_types[m].modeName.toLowerCase()]++;}
            
              if(options.hasOwnProperty(survey[i].families[j].members[k].trips[l].mode_types[m].modeName))
              options[survey[i].families[j].members[k].trips[l].mode_types[m].modeName].count++;
              else
              nulls++;
              }
...
function download_png() {
  ...
  return (
      ...
      { chart_type===0 && <div  className="chart-title">{props.title} </div>}
     ...
     { chart_type===0 &&<div className="chart-note"> 
       ...
       {chart_type===1 &&<div  className="chart-title">{props.title}  </div>}
       
       { chart_type===1 &&<div  className="chart-content doughnut">
          <Bar
          ref={(reference)=> chartref=reference} 
          data={chartData1} options={chartOptions1} />
        </div>}
