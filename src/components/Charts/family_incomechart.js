...
import options from './income_bracket';
...
function IncomeChart(props) {
  ...
  for(var i=0;i<survey.length;i++)
  {
   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
      // if(!income_range.hasOwnProperty(survey[i].families[j].familyIncome))
      // income_range[survey[i].families[j].familyIncome]=0;
      // if(survey[i].families[j].familyIncome!=='')
      // income_range[survey[i].families[j].familyIncome].count++;
      // else
      // {nulls++;
      //   income_range[''].count++;
      // }
      if(options.hasOwnProperty(survey[i].families[j].familyIncome))
      options[survey[i].families[j].familyIncome].count++;
      else
      nulls++;
      // income_range['null'].count++;
      // if(survey[i].families[j].familyIncome!=='')
      // income_range.survey[i].families[j].familyIncome.count+=1;
      
    }
  }
  options['Nil'].count+=nulls;
  ...
  const labels=[];
  const data1=[];
  if(options['Nil'].count!==0)
  {
    data.push(0);
    data1.push(options['Nil'].count);
    labels.push("Blanks/Nil");
  }
  Object.keys(options).forEach((value) => {
    if (options[value].count) {   
      if(value!=='Nil')   
...
function download_png() {
  ...
  return (
        ...
        <div id='family_income' className="charts-header">
          { chart_type===0  &&
