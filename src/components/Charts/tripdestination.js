...
import options from './trip_destination_options';
import  '../../Buttons.css';
// import html2canvas from "html2canvas";
const pdfConverter = require("jspdf"); 
function TripDestination(props) {
   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
      for(var k=0;k<survey[i].families[j].members.length;k++)
      {
        ...
        {
            for(var m=0;m<survey[i].families[j].members[k].trips[l].origin_destination.length;m++)
            {
              // if(survey[i].families[j].members[k].trips[l].origin_destination[m].destinationPlace===null)
              // {nulls++;
              ...
              // }
              // else
              //   {if(!destination.hasOwnProperty(survey[i].families[j].members[k].trips[l].origin_destination[m].destinationPlace))
              ...
              //   }
              if(options.hasOwnProperty(survey[i].families[j].members[k].trips[l].origin_destination[m].destinationPlace))
              options[survey[i].families[j].members[k].trips[l].origin_destination[m].destinationPlace].count++;
              else
              nulls++;
            }
        }
...
const buttonClasses = [ 
  ...
  return (
        ...
        <div id='trip_destination' className="charts-header">
          { chart_type===0  &&
