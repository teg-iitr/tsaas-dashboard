...
import options from './trip_fare_options';
import  '../../Buttons.css';
// import html2canvas from "html2canvas";
const pdfConverter = require("jspdf"); 
function TripFare(props) {
      ...
      for(var k=0;k<survey[i].families[j].members.length;k++)
      {
        for(var l=0;l<survey[i].families[j].members[k].trips.length;l++)
        {
            for(var m=0;m<survey[i].families[j].members[k].trips[l].origin_destination.length;m++)
            {
              ...
              //   }
              if(options.hasOwnProperty(survey[i].families[j].members[k].trips[l].origin_destination[m].fare))
              options[survey[i].families[j].members[k].trips[l].origin_destination[m].fare].count++;
              else
              nulls++;
            }
        }
...
const buttonClasses = [ 
  ...
  return (
        ...
        <div id='trip_fare' className="charts-header">
           { chart_type===0  &&
