...
function TwoWheelers(props) {
  ...
  for(var i=0;i<survey.length;i++)
  {
   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
            var x=survey[i].families[j].noOfTwoWheelers;
            x=x+'';
            //   if(x!=='0')
            // { 
  ...
  Object.keys(options).forEach((value) => {
    if (value) {
      labels.push(value+' Two Wheelers');
  ...
  var jsontocsv_array=[];
  jsontocsv_array.push({display:"Number of Two Wheelers",count:"Families Count"});
...
function download_png() {
  ...
  return (
        ...
        <div id='two_wheelers_count' className="charts-header">
          { chart_type===0  &&
