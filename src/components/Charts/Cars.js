...
function Cars(props) {
  ...
  for(var i=0;i<survey.length;i++)
  {
   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
            var x=survey[i].families[j].noOfCars;
            x=x+'';
            //   if(x!=='0')
            // { 
  ...
  var jsontocsv_array=[];
  jsontocsv_array.push({display:"Number of Cars",count:"Families Count"});
...
function download_png() {
  ...
  return (
        ...
        <div id='number_of_cars' className="charts-header">
          { chart_type===0  &&
