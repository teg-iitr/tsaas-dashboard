...
import options from './monthly_income_bracket';
// import html2canvas from "html2canvas";
const pdfConverter = require("jspdf"); 
function MemberIncomeChart(props) {
   ...
   for(var j=0;j<survey[i].families.length;j++)
    {
      for(var k=0;k<survey[i].families[j].members.length;k++)
      {
        ...
        //   age_bracket[''].count++;
        // }
        if(options.hasOwnProperty(survey[i].families[j].members[k].monthlyIncome))
        options[survey[i].families[j].members[k].monthlyIncome].count++;
        else
        nulls++;
...
function download_png() {
  ...
  return (
        ...
        <div id='member_income' className="charts-header">
          { chart_type===0  &&
...
export default MemberIncomeChart;
